...
from mfem.ser import getAdvectionEquation, RusanovFlux
...
def InitCond(x, out):
    out[0] = sin(pi*x[0])*sin(pi*x[1])

class Velocity(mfem.VectorPyCoefficient):
...
def run(problem=1,
    ...
    offsets = [k*fes.GetNDofs() for k in range(num_equations+1)]
    ...
    # #
    sol = mfem.GridFunction(fes, u_block.GetData())
    ...
    advection = getAdvectionEquation(fes, numericalFlux, b, IntOrderOffset)
    ...
    if (cfl > 0):
        ...
        z = mfem.Vector(advection.Width())
        advection.Mult(sol, z)

        dt = cfl * hmin / advection.getMaxCharSpeed() / (2*fes.GetMaxElementOrder()+1)
...
if __name__ == "__main__":
    ...
    parser = ArgParser(description='Advection equation')
